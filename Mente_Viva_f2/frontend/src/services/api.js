...
async function request(path, options = {}) {
  ...
  try {
    ...
    throw new Error('Nao foi possivel conectar com a API. Verifique se o backend esta rodando.');
  }

  const payload = await response.json().catch(() => null);

  if (!response.ok) {
    throw new Error(payload?.message || 'A API retornou um erro inesperado.');
  }
